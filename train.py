...
from data_conversion import create_loader, resolve_data_config, FastCollateMixup, mixup_target
...
Auto_Augmenation = True

from data_conversion.mv_dataset import MultiViewDataSet
import torchvision.transforms as transforms
from data_conversion.autoaugment import ImageNetPolicy
train_file = "./data/train.csv"
test_file = "./data/test.csv"
class_file = "./data/classes.csv"

if not Auto_Augmenation: # Normal augmentation
...
def run(args):
    ...
    if not os.path.exists(data_dir):
        ...
        exit(1)
    dataset_train = MultiViewDataSet(train_file, class_file, data_dir, transform=transform_train)    
    dataset_eval = MultiViewDataSet(test_file, class_file, data_dir,  transform=transform_eval)

    loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=1)
    if 0:
    ...
    for batch_idx, (inputs, target) in enumerate(loader):
        ...
        if not args.prefetcher:
            ...
            if args.mixup > 0.:
                ...
                inputs.mul_(lam).add_(1 - lam, inputs.flip(0))
                target = mixup_target(target, args.num_classes, lam, args.smoothing)
        #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")        
        inputs, target = inputs.cuda(device), target.cuda(device)
