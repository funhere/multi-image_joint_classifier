...
ResNeXt, SE-ResNeXt, SENet are newly added.
"""
import math
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 10, 'input_size': (3, 224, 224), 'pool_size': (7, 7),
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.drop_rate > 0.:
            pooled_view = F.dropout(x, p=self.drop_rate, training=self.training)
...
@register_model
def resnext50d_32x4d(pretrained=False, num_classes=1000, in_chans=3, dilated=True, **kwargs):
